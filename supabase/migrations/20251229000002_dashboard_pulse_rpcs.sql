-- Migration: 20251229000002_dashboard_pulse_rpcs.sql
-- Description: RPC functions for Dashboard "Pulse" tab (Weekly/Daily stats).

-- 1. Weekly Planning Compliance (vs Active Teachers)
-- Returns: total_enviadas (this week), total_profesores (active)
...
begin
  ...
  -- 'profiles' table uses 'activo' column (boolean)
  select count(*)
  ...
  -- 'planeaciones_enviadas' table linkage
...
-- Returns: count of incidents for TODAY, grouped by risk level
...
begin
  return query
  select 
    i.nivel_riesgo::text, -- Explicit cast to text to match return type
    count(*) as cantidad
  from incidencias i
...
-- 3. High Risk Students (SIAT style) - Wrapper/Specific for Dashboard
-- Returns: Top 5 students with highest count of incidents/absences
...
returns table (
  ...
  grupo text,
  motivo text, -- e.g. "3 Incidencias Críticas"
  nivel_riesgo text
)
language plpgsql
security definer
...
begin
  ...
  where i.plantel_id = p_plantel_id
    and i.nivel_riesgo in ('critico', 'alto') -- Valid enum values
...
declare
  ...
  v_presentes bigint;
begin
  -- Total active students (approximation using active groups)
  ...
  -- Count students marked as 'presente' TODAY in 'asistencia' table
  -- Assuming 'asistencia' table has 'alumno_id', 'fecha', 'estado' columns and 'presente' value
...
-- 5. Recently Active Teachers (Optional Pulse Widget)
...
begin
  ...
  select 
    p.id as profesor_id,
    p.full_name as nombre_completo, -- Fixed: profiles uses 'full_name' not 'nombre' || 'apellido'
