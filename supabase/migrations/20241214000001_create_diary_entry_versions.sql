...
-- Description: Creates table for diary entry version control and automatic versioning
-- Author: System
-- Date: 2024-12-14

-- Tabla para almacenar las versiones de las entradas del diario
...
CREATE INDEX IF NOT EXISTS idx_diary_entry_versions_version_number ON diary_entry_versions(version_number);
CREATE INDEX IF NOT EXISTS idx_diary_entry_versions_created_at ON diary_entry_versions(version_created_at);
...
-- Función para crear la versión inicial cuando se crea una entrada
CREATE OR REPLACE FUNCTION create_initial_diary_entry_version()
RETURNS TRIGGER AS $$
BEGIN
...
-- Trigger para crear la versión inicial después de insertar
DROP TRIGGER IF EXISTS create_initial_diary_entry_version_trigger ON diary_entries;
CREATE TRIGGER create_initial_diary_entry_version_trigger
    AFTER INSERT ON diary_entries
    FOR EACH ROW
    EXECUTE FUNCTION create_initial_diary_entry_version();
...
COMMENT ON TABLE diary_entry_versions IS 'Almacena las versiones históricas de las entradas del diario profesional';
COMMENT ON COLUMN diary_entry_versions.entry_id IS 'ID de la entrada del diario original';
COMMENT ON COLUMN diary_entry_versions.version_number IS 'Número de versión secuencial para cada entrada';
COMMENT ON COLUMN diary_entry_versions.version_created_at IS 'Fecha y hora cuando se creó esta versión';
...
BEGIN
    ...
    RAISE NOTICE 'Triggers created for automatic version management';
