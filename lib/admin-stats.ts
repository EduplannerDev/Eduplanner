...
/**
 * Obtiene historial de chats de ayuda usando la API admin para evitar problemas de RLS
 */
export async function getHelpChatLogs(): Promise<HelpChatLog[]> {
    try {
        ...
        return data as HelpChatLog[];
    } catch (error) {
        console.error('Error obteniendo logs de chat (API):', error);
        return [];
    }
}
