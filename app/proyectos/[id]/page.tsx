...
import { ArrowLeft, BookOpen, Users, Calendar, Target, Lightbulb } from 'lucide-react'
...
export default function ProyectoPage() {
  ...
  useEffect(() => {
    if (params.id) {
      cargarProyecto(params.id as string)
    }
  }, [params.id])

  const cargarProyecto = async (proyectoId: string) => {
    try {
  ...
  const cargarFases = async (proyectoId: string) => {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-6">{error || 'El proyecto solicitado no existe o no tienes permisos para verlo.'}</p>
          <Button onClick={() => router.push('/?section=proyectos')}>
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-6xl mx-auto p-6">
        ...
        <div className="mb-6">
          <Button 
            variant="outline" 
            onClick={() => router.push('/?section=proyectos')}
            className="mb-4"
          >
            <ArrowLeft className="h-4 w-4 mr-2" />
            Volver a Proyectos
